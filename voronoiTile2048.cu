                ...
                int nSeeds,int2 __restrict__ *seeds)
{
    ...
    data = ct::assume_aligned(data, 128_ic);
    //int x = ct::bid().x;
    //int y = ct::bid().y;
    auto [x,y,z] = ct::bid();
    // Attach a shape to the raw pointer
    auto data_span = ct::tensor_span{data, ct::extents{2048_ic,2048_ic}};
...
// Copy the 2D array from the device to the host memory
    ...
    // err = cudaMemcpy2D(h_data, width * sizeof(int), d_data, pitch, width * sizeof(int), height, cudaMemcpyDeviceT
oHost);
...
/*
    for(int i = 0;i < N;i++) {
      for(int j = 0;j < N;j++) {
        ...
        */
// Print 4 values that should have different closest seeds
    int i = 512;
    int j = 512;
    std::printf("512,512: %d\n", h_data[i*N+j]);
    i = 1536;
    std::printf("1536,512: %d\n", h_data[i*N+j]);
    i = 512;
    j = 1536;
    std::printf("512,1536: %d\n", h_data[i*N+j]);
    i = 1536;
    std::printf("1536,1536: %d\n", h_data[i*N+j]);
